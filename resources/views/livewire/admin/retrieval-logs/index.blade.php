<div>
    ...
    <div class="flex items-center gap-4 mb-6">
        <div class="flex-1 max-w-md">
            <flux:input icon="magnifying-glass" placeholder="Search queries…" wire:model.live.debounce.300ms="search" />
        </div>
        <flux:button icon="funnel" variant="outline" wire:click="$toggle('showFilters')">
            Filters
            @if ($filterNamespace || $filterLatencyMin !== null || $filterLatencyMax !== null || $filterDateFrom || $filterDateTo)
                <flux:badge size="sm" color="blue" class="ml-1">•</flux:badge>
            @endif
        </flux:button>
        @if ($search || $filterNamespace || $filterLatencyMin !== null || $filterLatencyMax !== null || $filterDateFrom || $filterDateTo)
    ...
    {{-- Filter Panel --}}
    @if ($showFilters)
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6 p-4 bg-zinc-50 dark:bg-zinc-900/50 rounded-lg border border-zinc-200 dark:border-zinc-800">
            <flux:field>
                <flux:label>Namespace</flux:label>
                <flux:select wire:model.live="filterNamespace">
            ...
            <flux:field>
                <flux:label>Min Latency (ms)</flux:label>
                <flux:input type="number" wire:model.live.debounce.500ms="filterLatencyMin" placeholder="0" />
            </flux:field>
            <flux:field>
                <flux:label>Max Latency (ms)</flux:label>
                <flux:input type="number" wire:model.live.debounce.500ms="filterLatencyMax" placeholder="5000" />
            </flux:field>
            <flux:field>
    ...
    <flux:table>
        <flux:table.columns>
            <flux:table.column sortable :sorted="$sortField === 'created_at'" :direction="$sortDirection" wire:click="sortBy('created_at')">Time</flux:table.column>
            <flux:table.column sortable :sorted="$sortField === 'query'" :direction="$sortDirection" wire:click="sortBy('query')">Query</flux:table.column>
            <flux:table.column>Namespace</flux:table.column>
            <flux:table.column sortable :sorted="$sortField === 'latency_ms'" :direction="$sortDirection" wire:click="sortBy('latency_ms')">Latency</flux:table.column>
        ...
        @empty
            <flux:table.row>
                <flux:table.cell colspan="7" class="text-center py-8 text-zinc-500 dark:text-zinc-400">
                    No retrieval logs found matching your criteria.
                </flux:table.cell>
            </flux:table.row>
        @endforelse
        </flux:table.rows>

...
| ----------------------- | ------------------ | ---------------------- |
| Documentation / Manuals | Semantic Search    | `VectorStore\Semantic`  |
| Web Content             | Semantic Search    | `VectorStore\Semantic`  |
| Markdown / PDF / TXT    | Full-text Scan    | `Filesystem\Filesystem` |
| SQL Databases           | Structured Query  | `Sql\Sql`              |
| YAML Configuration      | Structured Query  | `Yaml\Yaml`             |
| JSON Data               | Structured Query  | `Json\Json`             |
| Websites                | HTTP Fetch + Parse| `Web\Web`               |
...
*   **Pipeline Orchestration:** New documents are automatically routed through a multi-stage pipeline (Discover → Parse → Chunk → Enrich → Index) via batched queue jobs.
...
- **Hybrid Providers:** Filesystem, SQL, YAML, JSON, Web, Vector (Typesense), and Federation providers built in.
...
- **Provider SDK:** Extensible architecture with 8 contracts, `make:knowledge-provider` generator, auto-discovery via config, and full extension guide.

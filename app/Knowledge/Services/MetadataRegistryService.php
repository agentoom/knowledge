...
class MetadataRegistryService
{
    ...
    {
        $this->loaded = null; // invalidate in-process cache
        ...
        // Retention policy: keep only the last 50 registry versions (~500KB)
        $keepIds = MetadataRegistry::latest('id')->take(50)->pluck('id');
        MetadataRegistry::whereNotIn('id', $keepIds)->delete();
    ...
    {
        // In-process cache — avoids DB + Redis on every search within a request
        if ($this->loaded !== null) {
            return $this->loaded;
        }
        ...
        return $this->loaded = $latest->payload;
    }
